function discovery(input) {
  ...
  for (const plant in discoveredPlants) {
    if (discoveredPlants[plant]["Rating"] === 0) {
      discoveredPlants[plant]["Rating"] =
        discoveredPlants[plant]["Rating"].toFixed(2);
    } else {
      discoveredPlants[plant]["Rating"] = (
        discoveredPlants[plant]["Rating"] /
        discoveredPlants[plant]["RatingCount"]
      ).toFixed(2);
    }
    delete discoveredPlants[plant]["RatingCount"];
    console.log(
      `- ${plant}; Rarity: ${discoveredPlants[plant]["Rarity"]}; Rating: ${discoveredPlants[plant]["Rating"]}`
    );
  }
}
discovery([
